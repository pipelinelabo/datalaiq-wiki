...
em.AddSubTitle("Results from DatalaiQ infrastructure monitoring script")
from = "dataliq@example.com"
...
Note: The DatalaiQ client has a default request timeout of 5 seconds. For long running requests like system backups you should increase that timeout, but note that it is best practice to restore the original timeout when you've completed the long-running request; we have omitted that above for brevity.
