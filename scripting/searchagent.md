...
If `Insecure-Skip-TLS-Verify` is set to true, the search agent will *not* verify the validity of TLS certificates when connecting to an HTTPS-enabled DatalaiQ webserver. Use this option with care and see [the certificates documentation](/configuration/certificates) for more information.

**Insecure-Use-HTTP**

If `Insecure-Use-HTTP` is set to true, the search agent will attempt to communicate with the DatalaiQ webserver using plaintext HTTP rather than the default HTTPS. This option is set to true in the default configuration file because [DatalaiQ requires manual configuration to enable HTTPS](/configuration/certificates)
