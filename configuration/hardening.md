...
We also highly recommend keeping up-to-date with the latest DatalaiQ releases and occasionally checking in on the [changelog](/changelog/list).  If we encounter a security issue we will document it there.  We will also notify customers of critical security issues via the prescribed point of contact.
...
6. Enable HTTPS communication in the Search Agent [More Info](certificates)
...
The notable exception is the File Follower ingester which executes under the `admin` group so that it can tail log files in `/var/log`.  If you do not want *any* DatalaiQ component executing with elevated privileges we recommend not using the [File Follower](ingesters_list) and instead configure syslog to send data to the [Simple Relay](ingesters_list) ingester via TCP.  You may also alter the File Follower systemd unit file to execute using the `gravwell` group if you do not need to follow any controlled system log files.  Check out the system unit file section below for more info.
